...
CREATE TABLE `messages` (
    ...
    `content` MEDIUMTEXT NOT NULL COMMENT '消息内容',  -- 改为MEDIUMTEXT
    ...
    FULLTEXT INDEX `idx_content` (`content`(1000)) COMMENT '全文索引'  -- 注意：MEDIUMTEXT需要指定前缀长度
...
-- 插入默认模型配置
INSERT INTO `system_models` (`model_name`, `model_provider`, `model_type`, `api_endpoint`, `api_version`, `is_default`, `description`) VALUES
...
-- 创建管理员用户（密码：admin123）
INSERT INTO `users` (`username`, `password_hash`, `email`, `is_active`, `is_locked`) 
VALUES ('admin', '$2b$12$YourBcryptHashHere', 'admin@example.com', TRUE, FALSE);


-- 添加复合索引优化查询性能
ALTER TABLE `conversations` ADD INDEX `idx_user_model_time` (`user_id`, `model_id`, `created_at`);
ALTER TABLE `messages` ADD INDEX `idx_role_conversation` (`role`, `conversation_id`);
ALTER TABLE `user_model_configs` ADD INDEX `idx_enabled_priority` (`is_enabled`, `priority`);
ALTER TABLE `api_call_logs` ADD INDEX `idx_date_success` (`created_at`, `is_success`);
