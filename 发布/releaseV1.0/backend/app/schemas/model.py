...
# app/schemas/model.py - 在文件末尾添加

class UserModelConfigCreateRequest(BaseModel):
    """创建用户模型配置的请求模型"""
    model_id: int = Field(..., description="模型ID", gt=0)
    api_key: str = Field(..., min_length=10, max_length=500, description="API密钥")
    custom_endpoint: Optional[str] = Field(None, description="自定义端点URL")
    is_enabled: bool = Field(True, description="是否启用")
    priority: int = Field(0, ge=0, le=100, description="优先级（0-100，数值越大优先级越高）")
    temperature: Optional[float] = Field(None, ge=0.0, le=2.0, description="默认温度")
    max_tokens: Optional[int] = Field(None, gt=0, le=32768, description="默认最大token数")
