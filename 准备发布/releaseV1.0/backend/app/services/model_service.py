...
class ModelService:
    ...
    # app/services/model_service.py - 在 ModelService 类中添加
    ...
    def create_user_model_config(
        ...
        try:
            # 验证必填字段
            required_fields = ["model_id", "api_key"]
            ...
            # 检查是否已存在配置
            existing_config = self.user_config_repo.get_user_config_for_model(user_id, model_id)
            if existing_config:
                return {
                    "success": False,
                    "message": f"该模型配置已存在，请使用更新接口"
                }
            
            # 验证API密钥格式（根据不同提供商）
            api_key = config_data.get("api_key", "")
            provider = system_model.model_provider.lower() if system_model.model_provider else ""
            ...
            # 可选字段
            if "custom_endpoint" in config_data:
                create_data["custom_endpoint"] = config_data["custom_endpoint"]
            
            if "max_tokens" in config_data:
                create_data["max_tokens"] = config_data["max_tokens"]
            
            if "temperature" in config_data:
                create_data["temperature"] = config_data["temperature"]
            
            # 创建配置（通过模型路由服务以确保加密）
            from app.services.model_router import ModelRouterService
            model_router = ModelRouterService(self.db)
            
            result = model_router.update_user_model_config(user_id, create_data)
            
            if result["success"]:
                # 获取创建后的配置详情
                config_detail = self.get_user_model_config(user_id, model_id)
                return {
                    "success": True,
                    "message": "模型配置创建成功",
                    "data": config_detail
                ...
                return {
                    "success": False,
                    "message": f"配置创建失败: {result.get('message', '未知错误')}"
                }
                
        except Exception as e:
            logger.error(f"创建用户模型配置异常: {e}", exc_info=True)
            return {
                "success": False,
                "message": f"配置创建失败: {str(e)}"
            }

    
    # app/services/model_service.py - 在 ModelService 类中添加

    def get_system_model_details(self, model_id: int) -> Optional[Dict[str, Any]]:
        """获取系统模型详细信息"""
        system_model = self.system_model_repo.get_by_id(model_id)
        ...
        return {
            ...
            "model_type": system_model.model_type,
            "description": system_model.description,
            "api_endpoint": system_model.api_endpoint,
            "max_tokens": system_model.max_tokens,
            "rate_limit": system_model.rate_limit_per_minute,
            "is_available": system_model.is_available,
            "is_default": system_model.is_default,
            "created_at": system_model.created_at,
            "updated_at": system_model.updated_at
        }

    def get_user_config_summary(self, user_id: int) -> Dict[str, Any]:
        """获取用户配置摘要"""
        configs = self.user_config_repo.get_user_configs(user_id)
        
        total_count = len(configs)
        enabled_count = sum(1 for c in configs if c.is_enabled)
        has_api_key_count = sum(1 for c in configs if c.api_key or c.api_key_encrypted)
        
        # 按提供商统计
        provider_stats = {}
        for config in configs:
            if config.system_model:
                provider = config.system_model.model_provider
        ...
        return {
            ...
            "configs_with_api_key": has_api_key_count,
